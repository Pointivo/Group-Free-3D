...
def parse_option():
    ...
    parser.add_argument('--pred_save_path', default=None, required=True, help='Path to save predictions')
...
def evaluate_one_time(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, args, time=0):
    ...
    checkpoint_name = str(args.checkpoint_path).split('/')[-1].split('.')[0]
    # saving predictions
    if not os.path.exists(args.pred_save_path):
        os.mkdir(args.pred_save_path)
    with open(os.path.join(args.pred_save_path, f'{checkpoint_name}_predictions.pkl'), 'wb') as f:
        pickle.dump(batch_pred_map_cls_dict, f)
        print(f"{os.path.join(args.pred_save_path, f'{checkpoint_name}_predictions.pkl')} saved successfully !!")
    with open(os.path.join(args.pred_save_path, f'{checkpoint_name}_ground_truths.pkl'), 'wb') as f:
        pickle.dump(batch_gt_map_cls_dict, f)
        print(f"{os.path.join(args.pred_save_path, f'{checkpoint_name}_ground_truths.pkl')} saved successfully !!")
...
def eval(args, avg_times=5):
    ...
    evaluate_one_time(test_loader, DATASET_CONFIG, CONFIG_DICT, args.ap_iou_thresholds,
