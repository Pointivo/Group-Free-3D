...
def evaluate_one_time(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, args, time=0):
    ...
    with open(os.path.join(args.pred_save_path, 'predictions.pkl'), 'wb') as f:
        pickle.dump(batch_pred_map_cls_dict, f)
        print(f"{os.path.join(args.pred_save_path, 'predictions.pkl')} saved successfully !!")
    with open(os.path.join(args.pred_save_path, 'ground_truths.pkl'), 'wb') as f:
        pickle.dump(batch_gt_map_cls_dict, f)
        print(f"{os.path.join(args.pred_save_path, 'ground_truths.pkl')} saved successfully !!")
    for prefix in prefixes:
